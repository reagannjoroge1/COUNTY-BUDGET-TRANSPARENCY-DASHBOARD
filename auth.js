...
const Auth = (() => {
  ...
  async function init() {
    ...
    try {
      ...
      // Wrapped separately so a failure here doesn't abort onAuthStateChanged registration
      try {
        await firebaseAuth.setPersistence(firebase.auth.Auth.Persistence.LOCAL);
      } catch (persistErr) {
        console.warn('[Auth] setPersistence failed (non-fatal):', persistErr.code);
      }

      // Handle Google redirect result on page load (for signInWithRedirect flow)
      try {
        const result = await firebaseAuth.getRedirectResult();
        if (result && result.user) {
          toast(`Welcome, ${result.user.displayName || 'Nairobi Resident'}!`, 'success');
        }
      } catch (err) {
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      // Use redirect instead of popup — more reliable on GitHub Pages / static hosts
      await firebaseAuth.signInWithRedirect(provider);
      // Page will redirect to Google and return; result is handled in init() via getRedirectResult()
    } catch (err) {
